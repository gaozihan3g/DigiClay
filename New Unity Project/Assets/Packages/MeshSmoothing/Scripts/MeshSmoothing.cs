...
namespace mattatz.MeshSmoothingSystem {
	...
	public class MeshSmoothing {
		...
		//
		public static Mesh LaplacianFilter (Mesh mesh, int times, bool[] isFeaturePoint, Vector3 center, float radius) {
		...
		public static Vector3[] LaplacianFilter(Vector3[] vertices, int[] triangles, int times, bool[] isFeaturePoint, Vector3 center, float radius) {
		...
		static Vector3[] LaplacianFilter(Dictionary<int, VertexConnection> network, Vector3[] origin, int[] triangles, bool[] isFeaturePoint, Vector3 center, float radius) {
		...
		public static Mesh LaplacianFilter (Mesh mesh, int times, bool[] isFeaturePoint = null) {
			mesh.vertices = LaplacianFilter(mesh.vertices, mesh.triangles, times, isFeaturePoint);
			mesh.RecalculateNormals();
			mesh.RecalculateBounds();
			return mesh;
		}

		public static Vector3[] LaplacianFilter(Vector3[] vertices, int[] triangles, int times, bool[] isFeaturePoint) {
			var network = VertexConnection.BuildNetwork(triangles);
			for(int i = 0; i < times; i++) {
				vertices = LaplacianFilter(network, vertices, triangles, isFeaturePoint);
			}
			return vertices;
		}


		static Vector3[] LaplacianFilter(Dictionary<int, VertexConnection> network, Vector3[] origin, int[] triangles, bool[] isFeaturePoint = null) {
			Vector3[] vertices = new Vector3[origin.Length];
