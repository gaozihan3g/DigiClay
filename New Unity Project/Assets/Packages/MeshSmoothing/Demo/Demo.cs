	...
	public class Demo : MonoBehaviour {
		...
		void Start () {
			...
			case FilterType.HC:
				...
				break;
			case FilterType.Half:
				
				List<bool> fp = new List<bool> ();

				for (int i = 0; i < filter.mesh.vertexCount; ++i)
					fp.Add( (i < filter.mesh.vertexCount/2) ? true : false);
				
				filter.mesh = MeshSmoothing.LaplacianFilter(filter.mesh, times, fp.ToArray());
				break;
			}
		}
