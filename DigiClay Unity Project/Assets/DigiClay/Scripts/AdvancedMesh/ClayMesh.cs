...
namespace DigiClay
{
    ...
    public class ClayMesh
    {
        ...
        float AngleDelta
        {
            get
            {
                return 2f * Mathf.PI / (float)Column;
            }
        }
        ...
        public void UpdateMesh()
        {
            ...
            for (int i = 0; i < vertices.Length; ++i)
            {
				if (GetVertexTypeFromIndex (i) == VertexType.OuterSide) {
					...
					//get theta
					float angleTheta = AngleDelta * columnIndex;
					//get heightTheta
					float heightTheta = HeightDelta * rowIndex;
...
//				Debug.Log (string.Format("i:{0} originLength:{1:F3} \t sign:{2} length:{3:F3} weight:{4:F3}", i, originLength, sign, length, weight));
        ...
        void GenerateInnerBottom()
        {
            ...
            // Inner Bottom Center
			newVertices.Add(new Vector3(0f, HeightDelta, 0f));
            newUVs.Add(Vector2.zero);
